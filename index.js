...
try {
  ...
  // ぼやけ対策：Thin(100)からMedium(500)相当へ変更（ファイル名は環境に合わせて調整してください）
  // @fontsourceのデフォルトに合わせて400(Regular)か500(Medium)を推奨します。
  GlobalFonts.registerFromPath(path.join(fontBase, 'noto-sans-jp-japanese-500-normal.woff'), 'NotoSansJP-Medium');
...
async function generateWelcomeImage({ date, time, name }, W = 1920, H = 1080) {
  ...
  const MEDIUM = 'NotoSansJP-Medium'; // 視認性向上のため変更
  ...
  ctx.fillStyle = '#333333'; // 少し濃く
  ctx.fillText('Welcome', W / 2, 310);

  // --- 日付・時間 ---
  ctx.font = `${pt(36)}px "${MEDIUM}"`; // サイズと太さをアップ
  ctx.fillStyle = '#333333';
  ctx.fillText(`${date}　${time}`, W / 2, 430);

  // --- 名前 ---
  ctx.font = `${pt(64)}px "${MEDIUM}"`; // サイズと太さをアップ
  ctx.fillStyle = '#222222';
  ctx.fillText(name, W / 2, 560);

  // --- メッセージ ---
  ctx.font = `${pt(28)}px "${MEDIUM}"`; // サイズアップ
  ...
  // --- フッター ---
  ctx.font = `${pt(18)}px "${MEDIUM}"`;
  ctx.fillStyle = '#999999';
  ctx.fillText(COMPANY_NAME, W / 2, 1000);
...
client.on('messageCreate', async (message) => {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `<@${message.author.id}> !\n` +
        `${parsed.date.replace('/', '月')}日 ${parsed.time.replace(':', '時')}分 ${parsed.name}のウェルカム画像を作成したよ！\n\n` +
